	function WindowSizingEngine(){
		...
		this.calculate = function(){
			...
			if(this.oWidth > this.minWidth && this.oHeight > this.minHeight) {
				if (this.className == this.win.view.southEastLayerTagClass) {
					...
					this.oWidth = this.tmpGapOfWidth;
					if(this.oHeight >= 100) this.oTop = this.totalMouseY;
		...
		this.change = function(){
			...
			$("#" + this.tagIds["c"]).css("left",this.cLeft).css("top",this.hHeight+ this.hTop);
			$("#" + this.tagIds["c"]).css("width",this.oWidth - this.oBorderWidth);
			$("#" + this.tagIds["c"]).css("height",this.oHeight - this.oBorderWidth - this.hHeight);
			...
			$("#" + this.tagIds["ne"]).css("left", this.oWidth - this.resizeWidth).css("top", 0);
			$("#" + this.tagIds["sw"]).css("left", 0).css("top", this.oHeight - this.resizeHeight);
			$("#" + this.tagIds["se"]).css("left", this.oWidth - this.resizeWidth).css("top", this.oHeight - this.resizeHeight);
			$("#" + this.tagIds["n"]).css("width",this.oWidth - this.resizeWidth*2);
			$("#" + this.tagIds["e"]).css("left", this.oWidth - this.resizeWidth);
			$("#" + this.tagIds["e"]).css("height",this.oHeight - this.resizeHeight*2);
		...
		this.fullScreen = function(winTag){
			...
			if(this.win.fullScreen){
				...
				this.oWidth = this.win.view.preOWidth;
				this.oHeight = this.win.view.preOHeight;
