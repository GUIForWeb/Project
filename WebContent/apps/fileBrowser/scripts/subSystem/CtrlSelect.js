function CtrlSelect(){
	...
	this.choose = function(event){
		...
		if(this.tag["t"].children[0].innerHTML != ".." && this.tag["s"].prop("tagName")== "TR"){
			this.name = this.tag["t"].children[0].innerHTML;
			var json = {"name":this.name,"type":this.tag["t"].children[2].innerHTML};
			var flag = false;
		...
		this.va["selectedData"] = new API().has(this.va["selectedData"]).filter();
	}
